...
test('CourierDeactivated event sets active to false', () => {
  const testEvents = [
    {
      eventId: 'test-1',
      type: 'CourierRegistered',
  ...
  // Deactivated couriers should be filtered out
  expect(result.length).toBe(0);
});

test('profile merging when both employeeId and globalCourierId exist', () => {
  const testEvents = [
    {
      eventId: 'test-1',
      type: 'CourierRegistered',
  ...
  // Should merge into one profile with both IDs
...
test('out-of-order events: latest timestamp wins', () => {
  // Events are in reverse chronological order in the array
  // but the latest timestamp should still win
  const testEvents = [
    {
      eventId: 'test-1',
      type: 'CourierRegistered',
      courierId: 'SC-9999',
      timestamp: '2026-06-15T14:00:00Z', // latest
      data: {
        name: 'Test Courier',
        city: 'Munich',
        vehicle: 'ebike'
    ...
    },
    {
      ...
      type: 'CourierProfileUpdated',
      courierId: 'SC-9999',
      timestamp: '2026-06-15T12:00:00Z', // middle
      data: {
        city: 'Hamburg'
      }
    },
    {
      ...
      type: 'CourierProfileUpdated',
      courierId: 'SC-9999',
      timestamp: '2026-06-15T10:00:00Z', // earliest
      data: {
        city: 'Berlin'
      }
    }
  ...
  // Munich from the latest timestamp should win
  expect(result.length).toBeGreaterThan(0);
  const profile = result[0];
  expect(profile.city).toBe('Munich');
});
...
test('unknown event types are ignored', () => {
  const testEvents = [
    ...
    {
      eventId: 'test-2',
      type: 'zesrtghesrtgsagaregt',
      courierId: 'SC-9999',
      timestamp: '2026-06-15T10:00:00Z',
      data: {
        random: 'swfewrfgergrg'
      }
    }
  ];
  const testIdMapping = { 'SC-9999': 'GC-9999' };
  ...
  // Should still process the valid event and ignore unknown
  expect(result.length).toBeGreaterThan(0);
  const profile = result[0];
  expect(profile.name).toBe('Test Courier');
...
test('courier with no id-mapping has null globalCourierId', () => {
  const testEvents = [
    {
      eventId: 'test-1',
      type: 'CourierRegistered',
  ...
  const testIdMapping = {}; // Empty mapping
  const result = reconcileEvents(testEvents, testIdMapping);
  
  expect(result.length).toBeGreaterThan(0);
  const profile = result[0];
  expect(profile.employeeId).toBe('SC-9999');
  expect(profile.globalCourierId).toBeNull();
});
