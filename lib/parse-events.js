  ...
  Object.keys(eventsByCourier).forEach(courierId => {
    ...
    courierEvents.forEach(event => {
      ...
      switch (event.type) {
        ...
        // handle profile updates with latest timestamp wins
        case 'CourierProfileUpdated':
          if (event.data.city) timestamp > profile.city ? profile.city = event.data.city : null;
          if (event.data.vehicle) timestamp > profile.vehicle ? profile.vehicle = event.data.vehicle : null;
          break;
